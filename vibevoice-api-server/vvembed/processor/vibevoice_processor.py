    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        # Second check: if it's a local path, check the tokenizer_config.json
            ...
            tokenizer_config_path = os.path.join(
                language_model_pretrained_name, "tokenizer_config.json"
            )
            if os.path.exists(tokenizer_config_path):
                try:
                    ...
                    with open(tokenizer_config_path, "r") as f:
                        ...
                        if (
                            "Qwen" in tok_config.get("tokenizer_class", "")
                            or "qwen" in tok_config.get("model_type", "").lower()
                        ):
                            is_qwen_tokenizer = True
        ...
        if is_qwen_tokenizer:
            ...
            if os.path.exists(language_model_pretrained_name) and os.path.isdir(
                ...
                tokenizer_json = os.path.join(
                    language_model_pretrained_name, "tokenizer.json"
                )
                ...
                if os.path.exists(vocab_file) and os.path.exists(merges_file):
                    try:
                        ...
                        if os.path.exists(tokenizer_json):
                            logger.info(
                                f"Loading tokenizer with explicit files from {language_model_pretrained_name}"
                            )
                            logger.info(f"  vocab.json: {os.path.exists(vocab_file)}")
                            logger.info(f"  merges.txt: {os.path.exists(merges_file)}")
                            logger.info(
                                f"  tokenizer.json: {os.path.exists(tokenizer_json)}"
                            )
                            ...
                            logger.info(
                                "Tokenizer loaded successfully with explicit file paths"
                            )
                        else:
                            # Fall back to slow tokenizer if no tokenizer.json
                            logger.info(
                                "Loading slow tokenizer (no tokenizer.json found)"
                            )
                            # Remove parameters that might interfere with direct file loading
                            clean_kwargs = {
                                k: v
                        ...
                        kwargs.pop(
                            "cache_dir", None
                        )  # Remove cache_dir to avoid cache usage
                    ...
                    vocab_file = os.path.join(
                        language_model_pretrained_name, "vocab.json"
                    )
                    merges_file = os.path.join(
                        language_model_pretrained_name, "merges.txt"
                    )
                    ...
                    if os.path.exists(vocab_file) and os.path.exists(merges_file):
                        ...
                        logger = logging.getLogger("VibeVoice")
                        logger.info("Retrying tokenizer load with explicit file paths")
                        clean_kwargs = {
                            k: v
                            for k, v in kwargs.items()
                            if k
