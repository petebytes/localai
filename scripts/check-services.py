...
class ServiceHealthChecker:
    ...
    def check_all_services(self, verbose: bool = False) -> Dict[str, Dict]:
        ...
        for service in services:
            ...
            print(f"  {color}{symbol} {service:30} {state:15} {health_str}{Colors.ENDC}")
    ...
    def compare_state(self, state_file: Path, current_results: Dict[str, Dict]) -> bool:
        ...
        for service in sorted(all_services):
            ...
            if saved_state_val != current_state_val or saved_health != current_health:
                # State changed
                if current_state_val == 'running' and (current_health == 'healthy' or current_health == 'none'):
                    color = Colors.OKGREEN
                    symbol = '↑'
                    status = 'improved'
                ...
                print(f"  {color}{symbol} {service:30} {saved_state_val:15} → {current_state_val:15}{Colors.ENDC}")
                ...
                print(f"  {color}{symbol} {service:30} {current_state_val:15} (unchanged){Colors.ENDC}")
    ...
    def print_summary(self, results: Dict[str, Dict]):
        ...
        healthy = sum(1 for r in results.values()
                     ...
                     (r.get('health') == 'healthy' or r.get('health') == 'none'))
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        metavar='STATE_FILE',
        help="Save health status to JSON file"
    )
    parser.add_argument(
        '--compare',
        type=str,
    ...
    try:
        ...
        unhealthy = [name for name, info in results.items()
                     if info.get('state') != 'running' or
                     (info.get('health') not in ['healthy', 'none'] and info.get('health') != '')]

        if unhealthy:
            print(f"{Colors.WARNING}Warning: {len(unhealthy)} service(s) are not healthy:{Colors.ENDC}")
