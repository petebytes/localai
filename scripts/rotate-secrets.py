...
class SecretRotator:
    ...
    def generate_secret(self, secret_name: str, secret_config: dict) -> str:
        ...
        if secret_type == 'random':
            ...
            if not jwt_secret:
                raise ValueError(f"Cannot generate {secret_name}: {depends_on} not yet generated")
    ...
    def write_env_file(self, env_path: Path, env_vars: Dict[str, str], dry_run: bool = False):
    ...
    def create_backup(self, dry_run: bool = False) -> Path:
        ...
        if dry_run:
            print(f"{Colors.OKCYAN}[DRY RUN] Would create backup at: {backup_path}{Colors.ENDC}")
            return backup_path
        ...
        for backup_path in backup_dir.iterdir():
            ...
            try:
                ...
                if backup_date < cutoff_date:
                    if dry_run:
                        print(f"{Colors.OKCYAN}[DRY RUN] Would delete old backup: {backup_path}{Colors.ENDC}")
                    else:
                        shutil.rmtree(backup_path)
                        print(f"{Colors.WARNING}Deleted old backup: {backup_path}{Colors.ENDC}")
            except ValueError:
    ...
    def rotate_secrets(self, secret_names: List[str] = None, dry_run: bool = False):
        ...
        if secret_names:
            ...
            secrets_to_rotate = [
                name for name, config in secrets_config.items()
                if config.get('auto_rotate', False)
            ]

        print(f"{Colors.HEADER}Rotating {len(secrets_to_rotate)} secrets...{Colors.ENDC}")
        ...
        for secret_name in secrets_to_rotate:
            ...
            if depends_on and depends_on not in self.new_secrets:
                ...
                if depends_on in secrets_to_rotate:
                    dep_value = self.generate_secret(depends_on, secrets_config[depends_on])
                    if dep_value:
            ...
            if new_value:
                ...
                print(f"  {Colors.WARNING}⊘ Skipped {secret_name} (static/user-managed){Colors.ENDC}")
        ...
        for env_file in env_files_to_update:
            ...
            if updated_count > 0:
                ...
                print(f"  {Colors.OKGREEN}✓ Updated {env_file} ({updated_count} secrets){Colors.ENDC}")
    ...
    def print_summary(self):
        ...
        print(f"  2. Run: {Colors.OKCYAN}./scripts/restart-after-rotation.sh{Colors.ENDC}")
        print(f"  3. Verify services are healthy")
        print(f"  4. If issues occur, restore with: {Colors.WARNING}./scripts/restore-secrets.py{Colors.ENDC}")
        print()
